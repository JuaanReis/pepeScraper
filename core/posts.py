...
def get_post() -> list:
    try:
        with open("./data/boards.json", "rb") as f:
            data = json.loads(f.read())
        return [board["board"] for board in data["boards"]]
    except (FileNotFoundError, ValueError):
        boards = get_boards_api()
        if not boards:
            raise RuntimeError("Failed to download boards." if config.debug else "")
        with open("./data/boards.json", "rb") as f:
            data = json.loads(f.read())
        return [board["board"] for board in data["boards"]]
...
def get_post_thread(selected_boards: list[str] | None = None, workers=20) -> dict:
    ...
    def fetch_boards(b):
        ...
        threads = [t["no"] for page in catalog for t in page.get("threads", [])]
        return (b, threads)

    boards_iter = (
        tqdm(boards, mininterval=0.1, desc="Processing boards",
             bar_format=config.color_ansi + "{l_bar}{bar}{r_bar}" + "\033[0m", ncols=100)
        if not config.debug else boards
    )

    max_workers = min(workers, config.max_threads) * max(1, config.thread_multiplier)

    with ThreadPoolExecutor(max_workers=max_workers) as exe:
        return dict(exe.map(fetch_boards, boards_iter))
